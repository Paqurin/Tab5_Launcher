...
#ifdef CONFIG_IDF_TARGET_ESP32P4
    // CRITICAL FIX: Proper ESP32-C6 initialization sequence
    // Step 1: Power on ESP32-C6 coprocessor
    ESP_LOGI(TAG, "Step 1: Powering on ESP32-C6 coprocessor...");
    esp_err_t ret = esp_hosted_init_c6_power();
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "FATAL: Failed to power on ESP32-C6 coprocessor: %s", esp_err_to_name(ret));
        ESP_LOGE(TAG, "Check I2C bus and AXP2101 PMIC communication");
        return ret;
    }
    ESP_LOGI(TAG, "ESP32-C6 coprocessor powered on successfully");

    // Step 2: Reset ESP32-C6 coprocessor
    ESP_LOGI(TAG, "Step 2: Resetting ESP32-C6 coprocessor...");
    ret = esp_hosted_reset_c6();
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "FATAL: Failed to reset ESP32-C6 coprocessor: %s", esp_err_to_name(ret));
        ESP_LOGE(TAG, "Check GPIO 13 (SDIO1_D1) configuration and connections");
        esp_hosted_deinit_c6_power();  // Clean up power state
        return ret;
    }
    ESP_LOGI(TAG, "ESP32-C6 reset sequence completed");

    // Step 3: Initialize WiFi Remote (provides standard WiFi API via ESP-Hosted)
    ESP_LOGI(TAG, "Step 3: Initializing WiFi Remote (ESP-Hosted SDIO transport)...");
    wifi_init_config_t wifi_cfg = WIFI_INIT_CONFIG_DEFAULT();
    ret = esp_wifi_remote_init(&wifi_cfg);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "FATAL: WiFi Remote initialization failed: %s", esp_err_to_name(ret));
        ESP_LOGE(TAG, "Check ESP32-C6 firmware and SDIO connections (GPIOs 10-15)");
        ESP_LOGE(TAG, "Verify SDIO pins are correct: D0=12, D1=13, D2=14, D3=15, CMD=11, CLK=10");

        // Clean up on failure - DO NOT continue with corrupted state
        esp_hosted_deinit_c6_power();
        return ret;
    }
    ESP_LOGI(TAG, "WiFi Remote initialized successfully");
    ESP_LOGI(TAG, "WiFi hardware ready via ESP-Hosted SDIO transport on slot 1");
#endif
...
esp_err_t wifi_manager_init(wifi_status_callback_t status_callback) {
    ...
    // CRITICAL FIX: Ensure WiFi hardware is initialized with proper error handling
    esp_err_t ret = wifi_manager_ensure_hardware_initialized();
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "FATAL: Failed to initialize WiFi hardware: %s", esp_err_to_name(ret));
        ESP_LOGE(TAG, "WiFi functionality will be unavailable");
        // DO NOT continue - return error to caller
        return ret;
    }
...
esp_err_t wifi_manager_deinit(void) {
    ...
    ESP_LOGI(TAG, "Powering off ESP32-C6 coprocessor...");
    esp_hosted_deinit_c6_power();
#endif
