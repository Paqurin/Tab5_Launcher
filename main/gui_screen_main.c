...
void create_main_screen(void) {
    ...
    lv_obj_align(file_mgr_btn, LV_ALIGN_CENTER, 0, -120);
    ...
    lv_obj_align(fw_loader_btn, LV_ALIGN_CENTER, 0, -25);
    ...
    lv_obj_align(run_fw_btn, LV_ALIGN_CENTER, 0, 70);
    ...
    // Eject Firmware button - add between Run Firmware and Settings
    lv_obj_t *eject_fw_btn = lv_button_create(center_container);
    lv_obj_set_size(eject_fw_btn, lv_pct(95), 85);
    lv_obj_align(eject_fw_btn, LV_ALIGN_CENTER, 0, 165);
    apply_button_style(eject_fw_btn);
    lv_obj_add_event_cb(eject_fw_btn, main_menu_event_handler, LV_EVENT_CLICKED, (void*)(uintptr_t)4);
    
    lv_obj_t *eject_fw_label = lv_label_create(eject_fw_btn);
    // Check if firmware is available to determine button state
    if (firmware_loader_is_firmware_ready()) {
        lv_label_set_text(eject_fw_label, LV_SYMBOL_EJECT " Eject Firmware");
        lv_obj_set_style_text_color(eject_fw_label, THEME_WARNING_COLOR, 0);
    } else {
        lv_label_set_text(eject_fw_label, LV_SYMBOL_CLOSE " No Firmware");
        lv_obj_set_style_text_color(eject_fw_label, THEME_TEXT_MUTED, 0);
        lv_obj_add_state(eject_fw_btn, LV_STATE_DISABLED);
    }
    lv_obj_center(eject_fw_label);
    
    // Settings button - move down to accommodate eject button
    lv_obj_t *settings_btn = lv_button_create(center_container);
    lv_obj_set_size(settings_btn, lv_pct(95), 85);
    lv_obj_align(settings_btn, LV_ALIGN_CENTER, 0, 260);
