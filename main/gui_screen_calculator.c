...
void destroy_calculator_screen(void) {
    if (calculator_screen) {
        ESP_LOGI(TAG, "Destroying calculator screen...");

        // Disable events on screen to prevent corruption during deletion
        lv_obj_remove_event_cb(calculator_screen, NULL);

        // Remove all event callbacks from child objects to prevent dangling pointers
        uint32_t child_cnt = lv_obj_get_child_count(calculator_screen);
        for (uint32_t i = 0; i < child_cnt; i++) {
            lv_obj_t *child = lv_obj_get_child(calculator_screen, i);
            if (child) {
